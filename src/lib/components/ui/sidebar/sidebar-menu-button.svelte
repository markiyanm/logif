...
<script lang="ts">
	...
	const resolvedTooltipContent = $derived(tooltipContent ?? tooltip);
...
{:else}
	<Tooltip.Root>
		...
		<Tooltip.Content
			...
			{...tooltipContentProps}
		>
			{#if typeof resolvedTooltipContent === "string"}
				{resolvedTooltipContent}
			{:else if resolvedTooltipContent}
				{@render resolvedTooltipContent()}
			{/if}
		</Tooltip.Content>
	</Tooltip.Root>
{/if}
